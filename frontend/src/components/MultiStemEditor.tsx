...
export function MultiStemEditor({
  ...
  const playheadVisiblePct =
    clamp((playheadPct / 100 - visibleStartGlobal) / visibleRangeGlobal, 0, 1) *
    100;
    ...
    <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/25 p-4" role="region" aria-label="Stem mixer editor">
      ...
      <div className="flex items-center gap-3 flex-wrap" role="toolbar" aria-label="Mixer controls">
        ...
        <div className="flex items-center gap-1 rounded-xl border border-white/10 bg-black/20">
          ...
          <span className="px-1 text-xs text-white/50" aria-live="polite">
            {Math.round(zoom * 100)}%
          </span>
          <button
            type="button"
      ...
      >
        ...
        <span>
          ...
          <span className="text-white/75">Pitch</span>,{" "}
          <span className="text-white/75">EQ</span>,{" "}
          <span className="text-white/75">Amplitude</span>, or{" "}
          <span className="text-white/75">Time</span> above—each opens a side
          panel next to the timeline. Click the same button again to close it.
        </span>
      </p>
            ...
            <div className="flex flex-col gap-5 p-4">
              ...
              <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2">
                <span
                  ...
                  style={{ backgroundColor: "var(--stem-color)" }}
                />
              ...
              {activePanel === "pitch" && (
                <div className="space-y-4">
                  ...
                  <p className="text-[10px] leading-snug text-white/45">
                    Preview and export use one playback rate: pitch and time
                    stretch combine (resampling), matching your exported WAV —
                    not a separate &quot;pitch-only&quot; engine.
                  </p>
                </div>
              )}
              ...
              {activePanel === "eq" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/10">
                    <span className="col-span-2 text-[10px] font-semibold uppercase tracking-wider text-white/50">
                      Quick filters
                    </span>
                    {[
                      {
                        label: "Bass Boost",
                        vals: { eqLow: 8, eqMid: 0, eqHigh: 0 },
                      },
                      {
                        label: "Presence",
                        vals: { eqLow: 0, eqMid: 4, eqHigh: 3 },
                      },
                      {
                        label: "Air",
                        vals: { eqLow: -2, eqMid: 0, eqHigh: 6 },
                      },
                      {
                        label: "Reset",
                        vals: { eqLow: 0, eqMid: 0, eqHigh: 0 },
                      },
                    ].map(({ label, vals }) => (
                      <button
                        key={label}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        onStemStateChange(activeStem.id, {
                          mixer: { ...activeState.mixer, gain: 6 },
                        })
                      }
                      className="ghost-button rounded-lg border border-white/10 py-1.5 text-xs text-white/70 hover:text-white"
                    >
                ...
                <div className="space-y-4">
                  <div>
                    <div className="mb-2 flex justify-between text-[10px] font-semibold uppercase tracking-wider text-white/50">
                      ...
                      <span className="text-amber-300 font-mono">
                        {activeState.timeStretch.toFixed(2)}x
                      </span>
                    </div>
                    <input
                      type="range"
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      onStemStateChange(activeStem.id, { timeStretch: 1 })
                    }
                    className="ghost-button w-full rounded-xl border border-white/10 py-2 text-xs text-white/60 hover:text-white"
                  >
                    Reset
                  ...
                  <p className="text-[10px] leading-snug text-white/45">
                    Labels are duration-style (0.5x–2x). Playback rate is
                    2^(pitch/12) / timeStretch so preview matches export.
                  </p>
                </div>
              )}
            </div>
      ...
      >
        ...
        />
        <span>
          <span className="text-white/65">Channel strip:</span> the controls
          below apply to the{" "}
          <span className="text-white/75">selected stem</span> (switch with the
          tabs). Each waveform row still has quick solo, mute, and gain; trim
          handles are on the waveform itself.
        </span>
      </p>
