...
export async function getStemWaveform(url: string, bins: number): Promise<number[] | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      ...
      resolve(row?.data?.length === bins ? row.data : null);
    };
    req.onerror = () => reject(req.error);
...
export async function setStemWaveform(url: string, bins: number, data: number[]): Promise<void> {
  const db = await openDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_NAME, "readwrite");
    const req = tx.objectStore(STORE_NAME).put({ key: cacheKey(url, bins), data });
