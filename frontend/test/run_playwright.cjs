console.log('RUN_PLAYWRIGHT_START');
const fs = require('fs');
...
(async () => { console.log('IIFE_START');
  ...
  // Mark onboarding complete so upload UI is visible in local dev
  await context.addInitScript(() => { localStorage.setItem('burnt-beats-onboarding-complete', 'true'); });
  ...
  // Wait for the processing panel (may be gated by onboarding)
  await page.waitForSelector('[data-testid="processing-settings-panel"]', { timeout: 20000 }).catch(()=>{});
  const labelInput = page.getByLabel('Choose audio file');
  let fileInput = null;
  if ((await labelInput.count()) > 0) {
    fileInput = labelInput;
  } else {
    const genericInput = page.locator('input[type="file"]').first();
    if ((await genericInput.count()) > 0) fileInput = genericInput;
  }
  if (!fileInput) { console.error('No file input found in DOM'); }
  else { await fileInput.setInputFiles(wav); console.log('Uploaded file for separation'); }
  ...
  // Reuse the same input (assumes it accepts multiple) - pass file paths to avoid large buffer errors
  if (fileInput) {
    await fileInput.setInputFiles(files);
  } else {
    console.error('No file input available for bulk upload');
  }
