...
# FIX: valid union type
_vad_instance: SileroOnnxVAD | None = None


# FIX: valid union return type
...
def get_speech_timestamps(
    ...
    Returns None if VAD model unavailable or audio cannot be decoded.
    """
    vad = _get_vad()
    if vad is None:
        return None

    # FIX: avoid decode exceptions bubbling up to callers
    try:
...
def get_chunk_boundaries(
    ...
    for i, seg in enumerate(merged):
        ...
        gap = (
            (merged[i + 1]["start"] - seg_end)
            if i + 1 < len(merged)
            else (total_s - seg_end)
        )
    ...
    # OPTIONAL: avoid tiny trailing chunks that can waste time / upset separators
