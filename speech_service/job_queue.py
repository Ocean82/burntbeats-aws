"""Single-worker async queue for speech enhancement jobs."""
...
from speech_service.config import MAX_QUEUE_DEPTH
from speech_service.job_utils import write_progress

logger = logging.getLogger(__name__)

_queued: deque[dict[str, Any]] = deque()
_condition: asyncio.Condition | None = None
_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="speech-worker")
...
async def start_worker(run_fn: Callable[..., None]) -> None:
    ...
    logger.info("Speech job worker started")
...
async def enqueue_job(item: dict[str, Any]) -> None:
    if _condition is None:
        raise RuntimeError("Speech worker not started")
    ...
    while True:
        ...
        try:
            ...
            await loop.run_in_executor(
                ...
                out_dir,
                item.get("denoise", True),
                item.get("batch", False),
            )
        except Exception as e:
            logger.exception("Speech job %s failed", job_id)
            write_progress(
                out_dir,
                {
                    "status": "failed",
