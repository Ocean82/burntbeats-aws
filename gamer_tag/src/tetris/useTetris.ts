...
export function useTetris() {
  ...
  const lockPiece = useCallback(() => {
    ...
    // Easter egg: combo messages
    if (newCombo >= 3) {
      showMessage(`${newCombo}x COMBO! 🔥🔥🔥`);
    }

    // Easter egg: score milestones
    if (newScore >= 10000 && s.score < 10000) {
      showMessage("10K CLUB! you're basically pro now 😎", 3000);
    }
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'Enter':
          if (stateRef.current.gameOver || !stateRef.current.started) {
            startGame();
          }
          break;
      }
